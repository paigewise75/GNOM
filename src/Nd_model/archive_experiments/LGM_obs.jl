...
#     end
#     # Source column
#     LGM_εNdobs.source = fill("Du et al 2020", size(LGM_εNdobs,1))
#     # AIBECS mismatch expects "value" column
#     LGM_εNdobs.value = ustrip.(upreferred.(LGM_εNdobs.εNd))
#     LGM_εNdobs
# end  

## GEOTRACES data
const xDNdobs, xεNdobs = let
    # Flierdt et al. (2016) data
    println("Adding Flierdt et al. (2016) data...")
    DNdobs2, εNdobs2 = let
        register(DataDep(
            "Flierdt_etal_2016_Nd", # name 
            """
            ...
            """, # message
            "https://ndownloader.figshare.com/files/6294558", # remote path 
    ...
    # Source column
    εNdobs4.source = fill("Du et al 2020", size(εNdobs4,1))
    # AIBECS mismatch expects "value" column
    εNdobs4.value = ustrip.(upreferred.(εNdobs4.εNd))
    εNdobs4


    # Concatenate obs from all sources into single DataFrame
    xDNdobs = vcat(DNdobs1, DNdobs2, DNdobs3, cols=[:lat, :lon, :depth, :Nd, :value, :source])
    xεNdobs = vcat(εNdobs1, εNdobs2, εNdobs3, εNdobs4, cols=[:lat, :lon, :depth, :εNd, :value, :source])

    # Show how many obs are in each dataset for both Nd and εNd
    println("Number of observations for each dataset:")
    println(vcat(DataFrame(:variable=>:Nd, (Symbol(x)=>count(xDNdobs.source .== x) for x in ("van de Flierdt", "GEOTRACES IDP17", "post IDP17", "Du et al 2020"))...),
                 DataFrame(:variable=>:εNd, (Symbol(x)=>count(xεNdobs.source .== x) for x in ("van de Flierdt", "GEOTRACES IDP17", "post IDP17", "Du et al 2020"))...)))
