...
function plot_εNd_sources!(fig, fun)
    ...
    for (i, (s1,s2)) in enumerate(zip(sources, sources_iso))
        ...
        axs[1,i] = fig[1,i] = Axis(fig, backgroundcolor=seafloor_color)
        ax = axs[1,i]
    ...
    for (i, (s1,s2)) in enumerate(zip(sources, sources_iso))
        text!(axs[1,i], 0, 0, text=string(panellabels[i]), align = (:left, :bottom), offset = (2, 2), space = :relative, fontsize=20, font=labelfont, color=:white)
        ...
        text!(axs[1,i], 60, 45, text=string(s1)[3:end] * " Nd", fontsize=20, align=(:left,:bottom), offset = (4, -2), font=labelfont, color=:white)
...
for fun in (log10, identity)
    local fig = Figure(resolution = (2000, 1000), backgroundcolor=:white)
