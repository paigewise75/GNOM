...
function plot_obs_maps!(fig)
    ...
    sources = ["van de Flierdt", "GEOTRACES IDP17", "post IDP17", "Du et al 2020"]
    # for (i,source) in enumerate(sources)
    #     df = select(xDNdobs, [:lat, :lon])[xDNdobs.source .== source,:]
    #     scts[i] = myscatter!(ax, centerlon.(df.lon), df.lat; marker=mshp[i], color=mcol[i], strokewidth=1, markersize)
    # end
    for (i, source) in enumerate(sources)
        df = select(xDNdobs, [:lat, :lon])[xDNdobs.source .== source, :]
        lon_values = Float32.(centerlon.(df.lon))
        lat_values = Float32.(df.lat)
        scts[i] = myscatter!(ax, lon_values, lat_values; marker=mshp[i], color=mcol[i], strokewidth=1, markersize)
    end    
