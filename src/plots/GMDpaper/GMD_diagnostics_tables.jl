...
let
    ...
    df = DataFrame(
        ...
        Symbol("Nd Concentration (pM)")=>[totalaverage(DNdₖ*upreferred(uDNd).|>uDNd,grd)|>ustrip for DNdₖ in collect(DNdₖs)],
        Symbol("% Nd")=>[∫dV(DNdₖ,grd)/∫dV(DNd,grd)|>per100|>ustrip for DNdₖ in collect(DNdₖs)],
        Symbol("Residence Time (yr)")=>[∫dV(DNdₖ,grd)/∫dV(sₖ,grd)*s|>yr|>ustrip for (DNdₖ,sₖ) in zip(collect(DNdₖs), collect(sources))]
    )
    @show df
