...
function plot_horizontal_maps!(fig)
    ...
    for (i, Δz) in enumerate(depths_intervals)
        ...
        hms[1] = myscatter!(ax, centerlon.(DNdobs.lon[iobs]), DNdobs.lat[iobs]; color=DNdobs.value[iobs] * upreferred(uDNd) .|> uDNd .|> ustrip, colormap=Ndcmap, colorrange=Ndclims, markersize=10, strokewidth = 3)
        ...
        hms[2] = myscatter!(ax, centerlon.(εNdobs.lon[iobs]), εNdobs.lat[iobs]; color=εNdobs.value[iobs] * upreferred(uεNd) .|> uεNd .|> ustrip, colormap=εcmap, colorrange=εclims, markersize=10, strokewidth = 3)
        mapit!(ax, clon, mypolys(clon), color=:transparent, strokecolor=:black, strokewidth=1)
        mylatlons!(ax, latticks45, lonticks60)
        i≠length(depths_intervals) && hidexdecorations!(ax, ticks=false, grid=false)
